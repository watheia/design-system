...
export class ComponentID {
  ...
  // overload when providing scope separaetly, e.g. `fromObject({ name: 'button' }, 'watheia.base-ui')`
  ...
  static isValidObject(o: any): o is ComponentIdObj {
    return (
      typeof o === "object" && typeof o.name === "string" && typeof o.scope === "string"
    )
  ...
  static isEqual(
    ...
    let result =
      a.scope === b.scope &&
      a.toString({ ignoreVersion: opts.ignoreVersion }) ===
        b.toString({ ignoreVersion: opts.ignoreVersion })
  ...
  static isEqualObj(
    a: ComponentIdObj | undefined,
    b: ComponentIdObj | undefined,
    opts: EqualityOption = {}
  ): boolean {
    if (!a && !b) return true
    if (!a || !b) return false
