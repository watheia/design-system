...
```js live=true
<Button onClick={() => new Promise((res) => setTimeout(res, 1500))}>Click here to see loader</Button>
```
...
<>
  ...
  <Button onClick={() => new Promise((res) => setTimeout(res, 1000))}>(cannot) click me repeatedly</Button>
</>
```
